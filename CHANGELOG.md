...
- **No mail was ever parsed on aioimaplib 2.x**: the RFC822 literal in a FETCH response is a `bytearray`, and the `isinstance(line, bytes)` filter skipped it. `bytearray` is accepted now.
- Once a real mail was stored, every setup failed with `TypeError: can't subtract offset-naive and offset-aware datetimes` (mail dates carry a timezone, the store compared them with naive `datetime.now()`). Timestamps are normalised to aware UTC.
